...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ExceptionResponse> handleNoResource(NoResourceFoundException e) {
        log.warn("존재하지 않는 경로 요청 - {}", e.getResourcePath());
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ExceptionResponse.of("존재하지 않는 API 입니다. API 명세서(/swagger-ui/index.html)를 참고해주세요.", e.getResourcePath()));
    }
