...
public interface MessageControllerDocs {
    ...
    @Operation(summary = "메시지 커서 페이징 조회",
    ...
    MessagePageResponse getMessages(
            ...
            @Parameter(description = "커서(seq). 없으면 최신부터") Long cursor,
            @Parameter(description = "페이지 크기 (1~100)", example = "50") int limit,
            @Parameter(description = "조회 방향 (BEFORE/AFTER)", example = "BEFORE") MessageDirection direction);
}
